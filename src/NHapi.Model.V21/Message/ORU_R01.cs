...
public class ORU_R01 : AbstractMessage  {
    ...
    /// </summary>
    public ORU_R01(IModelClassFactory factory)
        : base(factory){
       init(factory);
    }
    ...
    private void init(IModelClassFactory factory) {
       try {
          ...
          add(typeof(ORU_R01_PATIENT_RESULT), true, true);
...
get{
    ...
    public ORU_R01_PATIENT_RESULT GetPATIENT_RESULT() {
       ...
       try {
          ret = (ORU_R01_PATIENT_RESULT)GetStructure("PATIENT_RESULT");
       } catch(HL7Exception e) {
          HapiLogFactory.GetHapiLog(GetType()).Error("Unexpected error accessing data - this is probably a bug in the source code generator.", e);
          throw new Exception("An unexpected error occurred",e);
       }
    ...
    public ORU_R01_PATIENT_RESULT GetPATIENT_RESULT(int rep) {
       return (ORU_R01_PATIENT_RESULT)GetStructure("PATIENT_RESULT", rep);
    }
...
get{
        var reps = -1;
        try {
            reps = GetAll("PATIENT_RESULT").Length;
        } catch (HL7Exception e) {
    ...
    public IEnumerable<ORU_R01_PATIENT_RESULT> PATIENT_RESULTs
    {
        get
        {
            for (var rep = 0; rep < PATIENT_RESULTRepetitionsUsed; rep++)
            {
                yield return (ORU_R01_PATIENT_RESULT)GetStructure("PATIENT_RESULT", rep);
            }
        }
    }
    ...
    {
        return AddStructure("PATIENT_RESULT") as ORU_R01_PATIENT_RESULT;
    }
    ...
    {
        RemoveStructure("PATIENT_RESULT", toRemove);
    }
    ...
    {
        RemoveRepetition("PATIENT_RESULT", index);
    }
